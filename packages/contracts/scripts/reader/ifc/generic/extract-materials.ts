...
export const extractMaterials = async (ifcApi: IfcAPI, modelID: number): Promise<EcotrackMaterial[]> => {
  ...
  for (let i = 0; i < qIDs.size(); i++) {
    ...
    for (const quantity of qset.Quantities) {
      ...
      if (ifcLine.__typename === "IFCQUANTITYVOLUME") volume_m3 = n(ifcLine?.VolumeValue?.value);
      if (ifcLine.__typename === "IFCQUANTITYWEIGHT") mass_kg = n(ifcLine?.WeightValue?.value);
    }
